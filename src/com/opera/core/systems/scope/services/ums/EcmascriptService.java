...
public class EcmascriptService extends AbstractService implements IEcmaScriptDebugger {
	...
	public Integer executeScriptOnObject(String using, int objectId) {
		...
		if(object == null || !(object instanceof com.opera.core.systems.scope.protos.EcmascriptProtos.Object))
			return null;
	...
	public Integer getObject(String using) {
		...
		return (reply.getValue().getType().equals(Type.OBJECT)) ? reply.getValue().getObject().getObjectID() : null;
	}
	...
	public void setRuntime(com.opera.core.systems.scope.protos.EsdbgProtos.RuntimeInfo runtime) {
		throw new UnsupportedOperationException("Not suppported without ecmascript-debugger");
	}
	...
	private void processNewRuntimes() {
		while(!runtimesQueue.isEmpty()) {
			...
			if(runtime != null) {
				if(runtime.getHtmlFramePath().equals("_top") && windowManager.getActiveWindowId() == runtime.getWindowID()) {
					setRuntime(runtime);
				}
