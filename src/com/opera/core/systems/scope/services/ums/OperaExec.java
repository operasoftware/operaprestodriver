...
public class OperaExec extends AbstractService implements IOperaExec {
	...
	public List<String> getActionList() {
		...
		ActionInfoList infoList = null;
		try {
			infoList = ActionInfoList.parseFrom(response.getPayload());
		} catch (InvalidProtocolBufferException e) {
			throw new WebDriverException("Error while parsing the actions list");
		}
	...
	public void action(String using, String... params) {
		...
		List<String> excludedActions = Arrays.asList(new String[] {"Close page", "Select all", "Delete"});
		if(!excludedActions.contains(using))
			actionBuilder.setWindowID(services.getWindowManager().getActiveWindowId());
		//type.setSpace("preserve");
		builder.addActionList(actionBuilder);
		executeCommand(ExecCommand.EXEC, builder);
	}
	...
	public ScreenShotReply containsColor(Canvas canvas, long timeout, OperaColors... colors){
		...
		ScreenWatcherResult result = null;		
		try {
			result = ScreenWatcherResult.parseFrom(response.getPayload());
		} catch (InvalidProtocolBufferException e) {
			throw new WebDriverException("Error while parsing the contains color result");
		}
