...
public class OperaExec extends AbstractService implements IOperaExec {
	...
	private Set<String> initActionList() {
		...
		ActionInfoList infoList = parser.parseResponse(ActionInfoList.class, response);
		return new HashSet<String>(infoList.getName());
	}
	...
	public ScreenShotReply containsColor(Canvas canvas, long timeout, OperaColors... colors){
		...
		ScreenWatcherReply reply = parser.parseResponse(ScreenWatcherReply.class, response);
