...
public class StpConnection implements Runnable {
	...
	private Selector initSelector(InetAddress hostAddress, int port) throws IOException {
		...
		logger.log(Level.INFO, "webdriver-opera v0.2.100505, ready to accept connections on port " + port);
		return selector;
	}
	...
	private void accept(SelectionKey key) {
		...
		try {
			...
			//close(); //if we are this stage we probably want to continue until we receive a valid key
			connectionHandler.onException(e);
		}
	}
	...
	public void run() {
		while (waitForKey) {
			try {
				...
				close();
				connectionHandler.onException(ex);
