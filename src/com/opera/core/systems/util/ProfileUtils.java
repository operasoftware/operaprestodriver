...
public class ProfileUtils {
  ...
  public ProfileUtils(String largePrefsFolder, String smallPrefsFolder,
      String cachePrefsFolder, OperaDriverSettings settings) {
  ...
  public boolean isMainProfile(String prefsPath) {
    ...
    if (isMac()) {
      ...
      if (absolutePrefsPath.startsWith(appSupport.getAbsolutePath())
          ...
          || absolutePrefsPath.startsWith(prefs.getAbsolutePath())) return true;
      ...
      if (absolutePrefsPath.startsWith(appFile.getAbsolutePath())) return true;

      // On XP:
      String homeDrive = System.getenv("HOMEDRIVE");
      String homePath = System.getenv("HOMEPATH");
      File homeOpera = new File(homeDrive + homePath
          + "\\Local Settings\\Application Data\\Opera");
      if (absolutePrefsPath.startsWith(homeOpera.getAbsolutePath())) return true;

      // In Vista/7:
      String localAppData = System.getenv("LOCALAPPDATA");
      File localAppDataFile = new File(localAppData + "\\Opera");
      if (absolutePrefsPath.startsWith(localAppDataFile.getAbsolutePath())) return true;
      ...
      // a/b/c/exe
      // a/b/c/profile
      if (prefsFile.equals(profileFolder)) return true;

    } else {

      /* *nix */
      File dotOpera = new File(path + "/.opera");
      if (/*platform nix && */prefsFile.equals(dotOpera)) return true;
  ...
  /**
   * Deletes prefs folders for Does nothing if prefs folders are default main
   * user profile
   */
  public void deleteProfile() {
    // Assuming if any of those are main profile, skip the whole delete
    if (isMainProfile(smallPrefsFolder) || isMainProfile(largePrefsFolder)
        || isMainProfile(cachePrefsFolder)) {
      return;
    }

    deleteFolder(smallPrefsFolder);
    if (!smallPrefsFolder.equals(largePrefsFolder)) deleteFolder(largePrefsFolder);
    if (!smallPrefsFolder.equals(cachePrefsFolder)
        && !largePrefsFolder.equals(cachePrefsFolder)) deleteFolder(cachePrefsFolder);
  }
